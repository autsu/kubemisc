...
```sh
go get -d k8s.io/samplecontroller
cd $GOPATH/src/k8s.io/samplecontroller
godep restore
```
...
**Prerequisite**: Since the sample-controller uses `apps/metav1` deployments, the Kubernetes cluster version should be greater than 1.9.

```sh
# assumes you have a working kubeconfig, not required if operating in-cluster
go build -o samplecontroller .
./samplecontroller -kubeconfig=$HOME/.kube/config
