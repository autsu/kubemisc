...
const controllerAgentName = "samplecontroller"
...
type Controller struct {
	...
	// （疑问：没有对 delete 事件进行处理，但是我测试后发现删除 foos 会一并删除其拥有的 deployment，这是如何做到的）
...
func NewController(
	...
	// Add samplecontroller types to the default Kubernetes Scheme so Events can be
	// logged for samplecontroller types.
	// 创建事件广播器
	// 将 samplecontroller 的类型信息（Foo）添加到默认 kubernetes Scheme，以便可以
	// 记录 samplecontroller 类型的事件
	utilruntime.Must(samplescheme.AddToScheme(scheme.Scheme))
	klog.V(4).Info("Creating event broadcaster")
	eventBroadcaster := record.NewBroadcaster()
	eventBroadcaster.StartStructuredLogging(0)
	eventBroadcaster.StartRecordingToSink(&typedcorev1.EventSinkImpl{Interface: kubeclientset.CoreV1().Events("")})
	// 基于新 Scheme 创建一个事件记录器 recoreder，用于记录来自 samplecontroller 的事件
...
//
// processNextWorkItem 将从工作队列中读取单个工作项并尝试通过调用syncHandler 来处理它。
...
//
// handleObject 将获取任何实现了 metav1.Object 接口的 resource 并尝试找到拥有它的 Foo resources，
// 然后它通过查看 metadata.ownerReferences 字段以获取适当的 OwnerReference 来做到这一点
// 然后它将要处理的 Foo resources 添加到队列，如果该 obj 没有适当的 OwnerReference，它将被简单的跳过
...
func (c *Controller) handleObject(obj interface{}) {
	...
	if ownerRef := metav1.GetControllerOf(object); ownerRef != nil {
		...
		// with it.
		// 如果该 obj 所属者的资源不是 Foo，我们不应该对它做更多的事情
